...
use crate::{
    ...
    testing::{
        ...
        tests::{
            get_pre_propose_info, no_init, proposal_multiple_contract, TestMultipleChoiceVote,
        },
    },
};
use cwd_pre_propose_multiple as cppm;

pub fn do_votes_bonded(
...
{
    let mut app = cw_multi_test::custom_app::<NeutronMsg, Empty, _>(no_init);
...
#[test]
fn test_vote_simple() {
    test_simple_votes(do_votes_bonded);
}

#[test]
fn test_vote_out_of_bounds() {
    test_vote_invalid_option(do_votes_bonded);
}

#[test]
fn test_no_overflow() {
    test_vote_no_overflow(do_votes_bonded)
}
...
#[test]
fn test_votes_tied() {
    test_vote_tied_rejected(do_votes_bonded)
}

#[test]
fn test_votes_none_of_the_above() {
    test_vote_none_of_the_above_only(do_votes_bonded)
}

#[test]
fn test_rounding() {
    test_tricky_rounding(do_votes_bonded)
}

#[test]
fn test_no_double_vote() {
    test_no_double_votes(do_votes_bonded)
}

#[test]
fn test_majority_half() {
    test_majority_vs_half(do_votes_bonded)
}

#[test]
fn test_pass_exact_quorum() {
    test_pass_exactly_quorum(do_votes_bonded)
}

#[test]
fn fuzz_votes_cw4_weights() {
    fuzz_voting(do_votes_bonded)
}
