...
pub fn is_allowed_to_pause(config: &Config, addr: &Addr) -> bool {
    addr == config.owner || addr == config.security_address
}

pub fn is_allowed_to_unpause(config: &Config, addr: &Addr) -> bool {
    addr == config.owner || addr == config.security_address
}
