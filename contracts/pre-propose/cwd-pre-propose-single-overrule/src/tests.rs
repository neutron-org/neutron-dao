...
use std::cell;
use cosmwasm_std::{coins, from_slice, to_binary, Addr, Binary, Coin, Empty, Uint128};
...
use cwd_proposal_single::msg::QueryMsg;
use cwd_voting::{
...
fn setup_default_test(
    ...
    let proposal_module_instantiate =
        get_default_proposal_module_instantiate(app);
    ...
    DefaultTestSetup {
        ...
        proposal_single,
        pre_propose,
        timelock_contract: Addr::unchecked("time_lock")
    }
}
...
fn make_proposal(
    ...
    let proposals: Vec<ProposalResponse> = app
        .wrap()
        .query_wasm_smart(&proposal_module, &cps::msg::QueryMsg::ListProposals { start_after: None, limit: None })
        .unwrap();
    // let id: u64 = app
    //     .wrap()
    ...
    let proposal: ProposalResponse = proposals.into_iter().next().unwrap();
        // app
        // .wrap()
        // .query_wasm_smart(
        //     proposal_module,
...
fn get_proposal_module(app: &App, module: Addr) -> Addr {
    app.wrap()
        .query_wasm_smart(module, &cwd_pre_propose_base::msg::QueryMsg::ProposalModule {})
        .unwrap()
}
...
fn test_native_permutation() {
    ...
    let proposal: ProposalResponse = app.wrap()
        .query_wasm_smart(proposal_single, &cps::msg::QueryMsg::Proposal { proposal_id: id})
        .unwrap();

    assert_eq!("Overrule proposal _number_ of _subdao_name_ subdao", proposal.proposal.title)
