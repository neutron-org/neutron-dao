...
pub fn execute_propose(
    ...
    let hooks = match proposal_creation_policy {
        ...
        ProposalCreationPolicy::Module { addr } => {
            let msg = to_json_binary(&PreProposeExecuteMsg::ProposalCreatedHook {
                proposal_id: id,
...
pub fn execute_execute(
    ...
    let response = {
        if !prop.msgs.is_empty() {
            let execute_message = WasmMsg::Execute {
                ...
                msg: to_json_binary(&neutron_subdao_core::msg::ExecuteMsg::ExecuteProposalHook {
                    msgs: prop.msgs,
                })?,
                funds: vec![],
            };
    ...
    let hooks = match proposal_creation_policy {
        ProposalCreationPolicy::Anyone {} => hooks,
        ProposalCreationPolicy::Module { addr } => {
            let msg = to_json_binary(&PreProposeExecuteMsg::ProposalCompletedHook {
                proposal_id,
