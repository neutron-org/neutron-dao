...
use neutron_drop_security_subdao_pre_propose::msg::{
    ExecuteMsgPause, ExecuteMsgUnpause, MigrateMsg,
};
...
pub(crate) const CONTRACT_NAME: &str = "crates.io:cwd-drop-security-subdao-pre-propose";
            ...
            for msg in &msgs {
                match msg {
                    CosmosMsg::Wasm(WasmMsg::Execute { msg, funds, .. }) => {
                        if (from_json::<ExecuteMsgPause>(msg).is_err()
                            && from_json::<ExecuteMsgUnpause>(msg).is_err())
                            || !funds.is_empty()
                        {
